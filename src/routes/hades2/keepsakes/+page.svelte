<script lang="ts">
    ...
    try {
      ...
      if (raw) {
        ...
        if (Array.isArray(parsed)) {
          unlockedKeepsakes = parsed.filter(
  ...
  $effect(() => {
    ...
    try {
      localStorage.setItem(
        ...
        JSON.stringify(unlockedKeepsakes),
      );
    } catch {}
  });
  ...
  let visibleKeepsakes = $derived(
    keepsakes.filter(([, details]) => {
      if (keepsakeFilter === "locked" && unlockedSet.has(details.name))
        return false;
      if (keepsakeFilter === "unlocked" && !unlockedSet.has(details.name))
        return false;
...
{#snippet descriptionRich(parts: DescriptionRich[], effectValues: string[])}
...
<Container>
  <div class="max-w-300 mx-auto text-[#e5f4e7] p-2 sm:p-3 font-serif">
    <header class="flex flex-col pb-1.5 border-b border-[#58ffa5]/25 mb-2">
      ...
      >
        Gifts from allies that grow stronger as you complete runs. Rank 2
        unlocks after 25 encounters, Rank 3 after 50. The Quickening of
        Sentimental Value Incantation halves these requirements. Heroic rank
        requires the
        <BoonHover name="Cherished Heirloom">Cherished Heirloom</BoonHover> duo boon
        from Demeter and Hera.
      </p>
      <p
        class="text-[0.7rem] sm:text-xs text-[#a4bea9] font-sans mt-0.5 tracking-wide leading-snug"
      >
    ...
    <nav
      class="flex gap-1.5 mb-2 flex-wrap justify-center"
      aria-label="Filter keepsakes"
    >
      <div
        class="flex flex-wrap gap-1.5 mt-2 justify-center"
        role="group"
        aria-label="Keepsake filters"
      >
    ...
    <div class="flex items-center gap-2.5 mb-2">
      <div class="mt-2 flex w-full items-center gap-2.5">
        <div
    ...
    <div class="keepsakesContainer">
      {#if visibleKeepsakes.length === 0}
        <p
          class="text-center text-[#8da693] font-sans text-sm py-8 tracking-wide"
        >
          {#if searchQuery.trim()}
            No keepsakes match "{searchQuery.trim()}".
          {:else if keepsakeFilter === "unlocked"}
            No unlocked keepsakes yet. Click a keepsake above to mark it as
            unlocked.
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-1.5">
          {#each visibleKeepsakes as [, details] (details.name)}
            ...
            <div
              ...
              aria-label="{details.name} keepsake — {isUnlocked
                ? 'unlocked, click to mark as locked'
                : 'locked, click to mark as unlocked'}"
            ...
            >
              ...
              <div
                class="flex flex-col gap-0.5 min-w-0 w-full z-10 relative text-center mt-1"
              >
                <span
